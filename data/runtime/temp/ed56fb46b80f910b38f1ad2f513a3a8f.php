...
<!-- <div class="bg_nav">
  ...
  <div class="banner">
    <div class="skitter" id="banner">
      <ul>
        ...
        <li><a href="<?php echo $field['links']; ?>" <?php echo $field['target']; ?>><img src="<?php echo $field['litpic']; ?>" alt="<?php echo $field['title']; ?>"
              class="random" /></a></li>
        <?php ++$e; endforeach; endif; else: echo htmlspecialchars_decode("");endif; $field = []; ?>
      </ul>
    </div>
  ...
  <!--产品中心子栏目——结束-->
  <?php  if(isset($ui_typeid) && !empty($ui_typeid)) : $typeid = $ui_typeid; else: $typeid = ""; endif; if(empty($typeid) && isset($channelartlist["id"]) && !empty($channelartlist["id"])) : $typeid = intval($channelartlist["id"]); endif;  if(isset($ui_row) && !empty($ui_row)) : $row = $ui_row; else: $row = 3; endif; $tagChannel = new \think\template\taglib\eyou\TagChannel; $_result = $tagChannel->getChannel($typeid, "son", "", ""); if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $e = 1;$__LIST__ = is_array($_result) ? array_slice($_result,0, $row, true) : $_result->slice(0, $row, true); if( count($__LIST__)==0 ) : echo htmlspecialchars_decode("");else: foreach($__LIST__ as $key=>$field): $field["typename"] = text_msubstr($field["typename"], 0, 100, false); $__LIST__[$key] = $_result[$key] = $field;$i= intval($key) + 1;$mod = ($i % 2 ); if(($key== 1)): ?>
      <div class="bg_gray white_bj">
      <?php else: ?>
      <div class=" bg_gray" >
      <?php endif; ?>

    <div class="width">
      <div class="home_pro home_pro_mod">
        <div class="home_title">
          <h2><?php echo $field['typename']; ?></h2>
          <div class="border"><?php echo $field['englist_name']; ?></div>
        </div>
        <!-- /标题 -->
        <div class="clear"></div>
        <div class="home_more">
          <span class="more"><a href="<?php echo $field['typeurl']; ?>">查看更多<i class="fa fa-angle-right"></i></a></span>
        </div>
        <!-- /更多 -->
        <ul id="con_one_<?php echo $i; ?>">
        ...
        <li>
          ...
          <div class="txt">
            <a href="<?php echo $field['arcurl']; ?>"
              title="<?php echo $field['title']; ?>"><?php echo $field['title']; ?></a><?php echo html_msubstr($field['seo_description'],0,40,true); ?>
          </div>
        </li>
        <?php ++$e; $aid = 0; endforeach; endif; else: echo htmlspecialchars_decode("");endif; $field = []; ?>
      </ul>
      <div class="clear"></div>
      <div class="home_more"> <span class="more"><a href="<?php  $__VALUE__ = isset($channelartlist["typeurl"]) ? $channelartlist["typeurl"] : "变量名不存在"; echo $__VALUE__; ?>">查看更多<i
              class="fa fa-angle-right"></i></a></span></div>
    </div>
  </div>
  <?php ++$e; endforeach; endif; else: echo htmlspecialchars_decode("");endif; $typeid = $row = ""; unset($channelartlist); ?>
  <!--新闻动态——结束-->
  <!--客户案例——开始-->
  <?php  if(isset($ui_typeid) && !empty($ui_typeid)) : $typeid = $ui_typeid; else: $typeid = "110"; endif; if(isset($ui_row) && !empty($ui_row)) : $row = $ui_row; else: $row = 10; endif; $tagChannelartlist = new \think\template\taglib\eyou\TagChannelartlist; $_result = $tagChannelartlist->getChannelartlist($typeid, "self"); if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $e = 1;$__LIST__ = is_array($_result) ? array_slice($_result,0, $row, true) : $_result->slice(0, $row, true); if( count($__LIST__)==0 ) : echo htmlspecialchars_decode("");else: foreach($__LIST__ as $key=>$channelartlist): $channelartlist["typename"] = text_msubstr($channelartlist["typename"], 0, 100, false); $__LIST__[$key] = $_result[$key] = $channelartlist;$i= intval($key) + 1;$mod = ($i % 2 ); ?>
  <div class="bg_gray">
    <div class="width">
      <div class="p10"></div>
      <div class="home_title">
...
<!--[if lt IE 9]>
  ...
  <script>
    $(function () {
      //Banner切换
      $('#banner').skitter({ dots: true, interval: 5000, animation: 'directionTop' });
    })
  </script>
